...
opcode_result_t dei(uint8_t phase, uint8_t ins, uint8_t controller0_buttons, uint8_t previous_stack_read, uint8_t previous_device_ram_read) {
	// t=T;            SET(1, 0) T = DEI(t);
	...
	if (phase == 0) {
		...
		if (~device_in_result.is_dei_done) {
			device_in_result = device_in(t8, phase - 2, controller0_buttons, previous_device_ram_read);
...
opcode_result_t dei2(uint8_t phase, uint8_t ins, uint8_t controller0_buttons, uint8_t previous_stack_read, uint8_t previous_device_ram_read) {
	...
	if (phase == 0) {
		...
		device_in_result = device_in(dei_param, current_dei_phase, controller0_buttons, previous_device_ram_read);
		if (device_in_result.is_dei_done) {
			current_dei_phase = 0;
			result.is_stack_write = 1;
...
eval_opcode_result_t eval_opcode_phased(
	...
	else if (opc == 0x16 /* DEI   */) { opc_result = dei(phase, ins, controller0_buttons, previous_stack_read, previous_device_ram_read); }
	else if (opc == 0x36 /* DEI2  */) { opc_result = dei2(phase, ins, controller0_buttons, previous_stack_read, previous_device_ram_read); }
